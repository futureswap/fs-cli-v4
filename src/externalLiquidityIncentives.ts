    ...
    .command(
      ...
      (yargs) =>
        externalLiquidityIncentivesOptions(commandWithSignerOptions(yargs))
          ...
          .option("permissions", {
            alias: "p",
            describe: "Permissions for the added accountant",
            choises: ["add", "adjust"],
            default: "add",
            require: true,
          }),
      async (argv) => {
        ...
        addAccountant(incentives, accountant, permissions);
      }
    )
    .command(
      "remove-accountant",
      ...
      async (argv) => {
        ...
        removeAccountant(incentives, accountant);
      }
    )
    .command(
      "upload",
      ...
      (yargs) =>
        uniswap
          .reportCommandOptions(commandWithSignerOptions(yargs))
      ...
      async (argv) => {
        ...
        const { networkId, "script-sha": scriptSha } = argv;
        const {
          priceStore,
...
const upload = async (
  ...
  if (BigNumber.from(incentivesTotal * 10 ** 18) > incentivesBalance) {
    console.log("ERROR:");
    console.log(
  ...
  for (const address of providerAddresses) {
    ...
    additions.push(new ProviderAddition(address, incentives));
  }
