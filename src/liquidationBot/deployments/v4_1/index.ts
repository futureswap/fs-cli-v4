...
export const init = ({
  ...
  async function getActiveTraders(provider: Provider) {
    try {
      ...
      const { openedTraders, closedTraders } = await getTradersActions(
        lastBlockRead,
        currentBlockNumber
      );
  ...
  async function getTradersActions(
    startBlock: number,
    endBlock: number
    ...
    for (
      ...
      for (const {
        args: { cpd },
      } of changePositionsEvents) {
        const { startAsset, startStable, totalAsset, totalStable } = cpd;
        const trader = cpd.trader as Trader;
